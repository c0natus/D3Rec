...
def evaluate(args, model, diffusion, loader, gt_items, consumed_items, topK, item_category, n_cate, temperature=1, is_best=False):
    ...
    with torch.no_grad():
        for batch_idx, (x_0, prob, prob_oracle) in enumerate(loader):
...
def get_paths(args):
    ...
    log_path = os.path.join(os.path.dirname(__file__), f'Best_models-C{args.drop_num}-{args.valid_ratio}'.replace(" ", ""))
